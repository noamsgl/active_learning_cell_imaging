...
if __name__ == "__main__":
    ...
    # --- model robustness --- #
    dummy_learner1 = LogisticRegression()
    dummy_learner2 = RandomForestClassifier()
    dummy_learner3 = svm.SVC(probability=True)
    learner_1 = ActiveLearner(
        estimator=LogisticRegression(),
        query_strategy=uncertainty_sampling
    )
    ...
    for i in range(range_epoch):
        ...
        dummy_learner1.fit(X_train_base_dummy1, y_train_base_dummy1)
        dummy_learner2.fit(X_train_base_dummy1, y_train_base_dummy1)
        dummy_learner3.fit(X_train_base_dummy1, y_train_base_dummy1)

        # evaluate the models
        dummy_pred1 = dummy_learner1.predict(X_test)
        dummy_pred2 = dummy_learner2.predict(X_test)
        dummy_pred3 = dummy_learner3.predict(X_test)
        st1_pred = learner_1.predict(X_test)
        st2_pred = learner_2.predict(X_test)
        st3_pred = learner_3.predict(X_test)

        if score_meth == 'auc':
            dummy_scores1.append(accuracy_score(dummy_pred1, y_test))
            dummy_scores2.append(accuracy_score(dummy_pred2, y_test))
            dummy_scores3.append(accuracy_score(dummy_pred3, y_test))
            ...
            dummy_scores1.append(average_precision_score(dummy_pred1, y_test))
            dummy_scores2.append(average_precision_score(dummy_pred2, y_test))
            dummy_scores3.append(average_precision_score(dummy_pred3, y_test))
            ...
            dummy_scores1.append(accuracy_score(dummy_pred1[np.where(y_test == 1)], y_test[np.where(y_test == 1)]))
            dummy_scores2.append(accuracy_score(dummy_pred2[np.where(y_test == 1)], y_test[np.where(y_test == 1)]))
            dummy_scores3.append(accuracy_score(dummy_pred3[np.where(y_test == 1)], y_test[np.where(y_test == 1)]))
        ...
        X_train_base_dummy = np.append(X_train_base_dummy1, [X_train_new_dummy1[0, :]], axis=0)
        y_train_base_dummy = np.concatenate([y_train_base_dummy1, np.array([y_train_new_dummy1[0]])], axis=0)
        X_train_new_dummy = X_train_new_dummy1[1:]
        y_train_new_dummy = y_train_new_dummy1[1:]
    ...
    plt.plot(list(range(range_epoch - 1)), dummy_scores1, label='dummy Logistic Regression')
    plt.plot(list(range(range_epoch - 1)), dummy_scores2, label='dummy Random Forest')
    plt.plot(list(range(range_epoch - 1)), dummy_scores3, label='dummy Support Vector')
    ...
    plt.savefig("models robustness vs dummy.pdf", bbox_inches='tight')
    plt.show()
